...
// Inline function to print a single element
static inline void print_single(size_t i, size_t n, const void *arr,
                                const char *fmt) {
    printf(fmt, *((const double *)arr +
                  i)); // we'll cast all to double for simplicity
    if (i != n - 1)
	printf(", ");
}
